...
class Client(Communicator):
    ...
    def CheckExeFile(self, Execution_Command, Executable_File):
        ...
        if os.path.isfile(Executable_File):
            if Execution_Command == './' or Execution_Command == 'sh':
                return Extension in ('sh', 'o')
            elif Execution_Command == 'java':
                return Extension == 'java'
            elif Execution_Command == 'python3' or Execution_Command == 'python':
                return Extension == 'py'
            else:
                return False
        else:
    ...
    def CreateChildProcess(self, Execution_Command, Executable_File):
        ...
        if self.CheckExeFile(Execution_Command, Executable_File):
            ...
            print(
                "ERROR : EITHER FILE {} DOES NOT EXIST OR THE EXECUTION COMMAND TO RUN THE FILE {} IS INCORRECT"
                .format(Executable_File, Execution_Command)
            )
    ...
    def RecvDataFromServer(self):
        ...
        if data is None:
            ...
            # Python3: socket recv gives bytes → decode before json.loads
    ...
    def RecvDataFromProcess(self):
        """Receives Data from the process (pipe)."""
...
def game_loop(game, args):
    ...
    if args.exe.endswith('.py'):
        client.CreateChildProcess('python3', args.exe)
