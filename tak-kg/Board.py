...
class Board(object):
            ...
            for c in range(self.n):
                ...
                for i, elem in enumerate(board_list[idx]):
                    if elem[1] == 'F':
                        ...
                        self.draw_capstone(
                            elem,
                            x_origin + Board.FLAT_SIZE / 2,
                            y_origin + Board.FLAT_SIZE / 2 - max(0, i - 1) * 5
                        )

    def draw_flat(self, elem, x, y):
        fill = ["#ecf0f1", "#34495e"]
        self.canvas.create_rectangle(
            x, y, x + Board.FLAT_SIZE, y + Board.FLAT_SIZE,
            fill=fill[elem[0]], outline="#000000"
        )
    ...
    def draw_wall(self, elem, x, y):
        ...
        self.canvas.create_polygon(
            x + 7, y + 2, x + 28, y + 23, x + 23, y + 28, x + 2, y + 7,
            fill=fill[elem[0]], outline="#000000"
        )
        ...
        for r in range(self.n):
            for c in range(self.n):
                ...
                if r % 2 == c % 2:
                    ...
                    fill = "#C2DDF6"
                self.canvas.create_rectangle(
                    x, y, x + Board.SQUARE_SIZE, y + Board.SQUARE_SIZE,
    ...
    def draw_tiles_remaining(self, player_list):
        ...
        self.canvas.create_rectangle(
            x_origin - 30, y_origin - 80, x_origin + 30, y_origin - 20,
            fill="#ecf0f1", outline="#000000"
        )
        self.canvas.create_text(
            (x_origin, y_origin - 50),
            text=str(player_list[0].flats),
            font=("Arial", self.FONT_SIZE),
            fill="#34495e"
        )
        ...
        self.canvas.create_rectangle(
            x_origin - 30, y_origin - 80, x_origin + 30, y_origin - 20,
            fill="#34495e", outline="#ecf0f1"
        )
        self.canvas.create_text(
            (x_origin, y_origin - 50),
            text=str(player_list[1].flats),
            font=("Arial", self.FONT_SIZE),
            fill="#ecf0f1"
        )
...
if __name__ == "__main__":
    # minimal runnable demo (no Game dependency)
    n = 7
    window_width = 2 * Board.HOR_SHIFT_MARGIN + n * Board.SQUARE_SIZE
    window_height = Board.VER_SHIFT_MARGIN + n * Board.SQUARE_SIZE + 150

    root = Tk()
    root.title("Tak Board")
    canvas = Canvas(root, width=window_width, height=window_height, bg="#2c3e50")
    canvas.pack()

    b = Board(n, canvas, window_height, window_width)
    b.draw_squares()
    b.draw_board_labels()
    root.mainloop()