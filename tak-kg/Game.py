...
class Game:
    ...
    def check_road_win(self, player):
        ...
        def check_road_win_inner(player, direction):
            ...
            if direction == 'ver':
                ...
                for i in range(self.n):
                    if (len(self.board[i * self.n]) > 0) and (self.board[i * self.n][-1][0] == player) and (
                            self.board[i * self.n][-1][1] != 'S'):
            ...
            while len(dfs_stack) > 0:
                ...
                for nbr in nbrs:
                    if (nbr not in visited) and (len(self.board[nbr]) > 0) and (self.board[nbr][-1][0] == player) and (
                            self.board[nbr][-1][1] != 'S'):
                        dfs_stack.append(nbr)
                        visited.add(nbr)
            return False

        return check_road_win_inner(player, 'hor') or check_road_win_inner(player, 'ver')

    def get_neighbours(self, square):
        """Generate a list of neighbours for a given square. Returns empty if square is invalid."""
    ...
    def calculate_score(self, player):
        ...
        count_1 = 0
        count_2 = 0
        for i in range(self.total_squares):  # FIXED: self.total_squares, not total_squares
            if len(self.board[i]) > 0 and self.board[i][-1][0] == 0 and self.board[i][-1][1] != 'S':
                count_1 += 1
            elif len(self.board[i]) > 0 and self.board[i][-1][0] == 1 and self.board[i][-1][1] != 'S':
                count_2 += 1
